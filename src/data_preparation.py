...
def get_data_splits(to_csv=False, columns=["text", "age", "gender"], buckets=True, bins=None):
    ...
    Args:
        ...
        buckets (bool): If True, convert ages to bucket labels.
        bins (list): Age bin boundaries for bucketing.
    Returns:
    ...
    data_train, data_test = dataset["train"], dataset["validation"]
    data_train, data_val = data_train.train_test_split(test_size=0.1).values()
    
    if buckets:
        bins = bins or [0, 18, 23, 27, 33, 37, 43, 47, 53, 57, 100]
        
        def bucketize_age(example):
            example["age"] = int(np.digitize(example["age"], bins) - 1)
            return example
        
        data_train = data_train.map(bucketize_age)
        data_val = data_val.map(bucketize_age)
        data_test = data_test.map(bucketize_age)
